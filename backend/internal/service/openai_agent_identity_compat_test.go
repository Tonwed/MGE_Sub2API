...
func TestOpenAIAgentIdentityPassthroughKeepsSessionAndPromptCacheHeaders(t *testing.T) {
	...
	require.Equal(t, scopeOpenAICodexSessionValue(account, 0, "client-session"), req.Header.Get("session_id"))
	require.Equal(t, req.Header.Get("session_id"), req.Header.Get("session-id"))
	require.Equal(t, scopeOpenAICodexSessionValue(account, 0, "client-conversation"), req.Header.Get("conversation_id"))
